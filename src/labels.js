    ...
    if (e) {
      ...
      res.json({
        ...
        'labels': labels.map(function(key) {
          return key.substr(7 + req.params.user.length);
        })
      });
    }
  });
    ...
    if (e) {
      ...
      redis.zadd('label:' + req.user + '/' + req.params.label, req.body.score, 'article:' + req.body.hash, function(e) {
        if (e) {
          ...
          if (req.params.label === 'read') {
            redis.zrem('label:' + req.user + '/unread', 'article:' + req.body.hash, function(e) {
              if (e) {
                res.status(500).json({
                  'success': false,
                  'error': {
...
exports.label.get = function(req, res) {
  if (req.params.label === 'unread') {
    redis.zrange('label:' + req.params.user + '/' + req.params.label, 0, -1, function(e, articles) {
      if (e) {
        res.status(500).json({
          'success': false,
          'error': {
        ...
        res.json({
          'success': true,
          'articles': articles.map(function(key) {
            return key.substr(8);
          })
        });
      }
    ...
    redis.zrevrange('label:' + req.params.user + '/' + req.params.label, 0, -1, function(e, articles) {
      if (e) {
        res.status(500).json({
          'success': false,
          'error': {
