...
exports.get = function(req,res){
  redis.smembers('labels:'+req.params.user,function(e,labels){
    if (e) res.json({'success':false,'error':{'type':'Redis Error','message':"Couldn't get labels for "+req.params.user}},500)
  ...
  redis.sadd('labels:'+req.user,'label:'+req.user+'/'+req.params.label,function(e){
    if (e) res.json({'success':false,'error':{'type':'Redis Error','message':"Couldn't add label:"+req.user+"/"+req.params.label+" to labels:"+req.user}},500)
      ...
      redis.zadd('label:'+req.user+'/'+req.params.label, req.body.score,'article:'+req.body.hash,function(e){
        if (e) res.json({'success':false,'error':{'type':'Redis Error','message':"Couldn't add article:"+req.body.hash+" to label:"+req.user+"/"+req.params.label}},500)
        else {
          if (req.params.label==='read') redis.zrem('label:'+req.user+'/unread','article:'+req.body.hash,function(e){
            if (e) res.json({'success':false,'error':{'type':'Redis Error','message':"Couldn't remove article:"+req.body.hash+" from label:"+req.user+"/unread"}},500)
...
exports.label.get = function(req,res){
  if (req.params.label === 'unread') redis.zrange('label:'+req.params.user+'/'+req.params.label,0,-1,function(e,articles){
    if (e) res.json({'success':false,'error':{'type':'Redis Error','message':"Couldn't get articles labelled "+req.params.label}},500)
